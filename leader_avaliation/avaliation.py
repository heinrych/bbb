...
def wait_and_click_votar_novamente(page, max_wait_s=600):
    """Aguarda o botao 'Votar Novamente' aparecer e clica."""
    deadline = time.time() + max_wait_s
    while time.time() < deadline:
        try:
            if click_votar_novamente(page):
                return True
        except Exception:
            pass
        time.sleep(0.8)
...
def clear_browser_cache_only(page):
    """Limpa cache/storage sem apagar cookies (preserva login)."""
    try:
        clear_page_cache(page)
    except Exception:
        pass
...
def recreate_page_after_captcha(page, playwright):
    ...
    for attempt in range(3):
        try:
            ...
            # fecha a pagina antiga somente depois da nova estar pronta
            try:
...
def handle_captcha_and_refresh(page, playwright):
    """Tenta resolver captcha e recria a página se necessário"""
    try:
        # Primeiro tenta clicar no captcha para abrir
        for i in range(VOTAR_NOVAMENTE_RETRY):
            ...
            if i + 1 >= VOTAR_NOVAMENTE_RETRY:
                raise Exception("hCaptcha não detectado após várias tentativas. Continuando sem clicar.")
        ...
        for i in range(VOTAR_NOVAMENTE_RETRY):
            ...
            try:
                ...
                if page.locator("text=Seu voto").count() > 0:
                    ...
                    # Recria a página após captcha resolvido
                    page = recreate_page_after_captcha(page, playwright)
                    return page
                ...
                if page.locator("button:has-text('Votar Novamente')").count() > 0:
                    ...
                    if not wait_and_click_votar_novamente(page, max_wait_s=600):
                        print("Nao encontrei o botao 'Votar Novamente' dentro do tempo limite.")
                    else:
                        time.sleep(random.uniform(1, 3))
                        clear_hcaptcha_cookies(page)
                        print("Fluxo reiniciado com 'Votar Novamente'.")
    ...
    with sync_playwright() as p:
        ...
        while True:
            ...
            try:
                ...
                # a cada MAX_INTERATIONS_NOW interações recria a página para evitar acúmulo de estado
